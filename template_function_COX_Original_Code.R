COX_Original_Code <- function(MacacaMetadata, P181_wk13, VB013_wk13) {
...
studies <- c("P181", "VB013")
timepoints <- list()
# timepoints[[1]] <- c("BASAL", "wk 13")
##timepoints[[1]] <- timepoints[[2]] <- timepoints[[3]] <- c("wk 13") # fixing inner loop, kludge
timepoints[[1]] <- timepoints[[2]] <- c("wk 13") # P181 and VB013

names(timepoints) <- c("P181", "VB013") # the name of the study to call the list of timepoints
...
for(study in studies)
{
  ...
  for(timepoint in timepoints[[study]]) #"BASAL",
  {
    ...
    sigout <- subset(coxoutput, p < 0.05 | regr_p < 0.05)
    write.csv(sigout, file = paste("sigvars cox and regr 2021",
                                   study, timepoint, analysis.text,  ".csv", sep = "_"))
    write.csv(coxoutput, file = paste("allvars cox and regr 2021",
                                   study, timepoint, analysis.text,  ".csv", sep = "_"))
  }
}
# write.csv(coxall, file = "concatenate cox rslts scld no delta.csv")
...
##return(list(value=NewSO))
}

print("template_function_COX_Original_Code.R #########################################################################")
...
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_P181_wk13<-readRDS(paste0(rds_output,"/var_P181_wk13.rds"))
Input_is_Seurat_count <- 0
for(item in var_P181_wk13){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_P181_wk13<-as.data.frame(var_P181_wk13)}else{var_P181_wk13 <- var_P181_wk13}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_VB013_wk13<-readRDS(paste0(rds_output,"/var_VB013_wk13.rds"))
Input_is_Seurat_count <- 0
for(item in var_VB013_wk13){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_VB013_wk13<-as.data.frame(var_VB013_wk13)}else{var_VB013_wk13 <- var_VB013_wk13}
invisible(graphics.off())
var_COX_Original_Code<-COX_Original_Code(var_MacacaMetadata,var_P181_wk13,var_VB013_wk13)
invisible(graphics.off())
saveRDS(var_COX_Original_Code, paste0(rds_output,"/var_COX_Original_Code.rds"))
