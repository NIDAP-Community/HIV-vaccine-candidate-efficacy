...
Meta_VB013 <- function(MacacaMetadata) {
    library(tidyverse)
    tab <- MacacaMetadata
    if(TRUE){
        x=paste0("^", c("VB013"), "$", collapse = "|")    
    }
    else{
        x=paste(c("VB013"),collapse="|")
    }
    if(TRUE){
...
print("template_function_Meta_VB013.R #########################################################################")
...
var_Meta_VB013<-Meta_VB013(var_MacacaMetadata)
invisible(graphics.off())
saveRDS(var_Meta_VB013, paste0(rds_output,"/var_Meta_VB013.rds"))
