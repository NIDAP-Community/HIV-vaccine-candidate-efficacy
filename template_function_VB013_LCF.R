...
VB013_LCF <- function(Normalized_Data, Meta_VB013) {
    ...
    if (TRUE) {
        ...
        metadata <- dplyr::collect(Meta_VB013)
        rownames(metadata) <- metadata$sampleID
        tcounts <- merge(metadata["ADJUVANT"], tcounts, by="row.names")
        tcounts$Row.names <- NULL
        melted <- melt(tcounts, id.vars="ADJUVANT")
        tcounts.tot <- summarise(group_by(melted, ADJUVANT, variable), sum=sum(value))
    ...
    Sampinfo <- dplyr::collect(Meta_VB013)
    ...
    if(FALSE){
    df.m %>% mutate(colgroup = Sampinfo[variable,"ADJUVANT"]) -> df.m
...
print("template_function_VB013_LCF.R #########################################################################")
...
var_VB013_LCF<-VB013_LCF(var_Normalized_Data,var_Meta_VB013)
invisible(graphics.off())
saveRDS(var_VB013_LCF, paste0(rds_output,"/var_VB013_LCF.rds"))
