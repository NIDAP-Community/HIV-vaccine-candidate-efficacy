COX_VB013 <- function(MacacaMetadata, Norm_VB013) {
...
names(timepoints) <- c("VB013") # the name of the study to call the list of timepoints
...
print("template_function_COX_VB013.R #########################################################################")
...
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_Norm_VB013<-readRDS(paste0(rds_output,"/var_Norm_VB013.rds"))
Input_is_Seurat_count <- 0
for(item in var_Norm_VB013){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_Norm_VB013<-as.data.frame(var_Norm_VB013)}else{var_Norm_VB013 <- var_Norm_VB013}
invisible(graphics.off())
var_COX_VB013<-COX_VB013(var_MacacaMetadata,var_Norm_VB013)
invisible(graphics.off())
saveRDS(var_COX_VB013, paste0(rds_output,"/var_COX_VB013.rds"))
