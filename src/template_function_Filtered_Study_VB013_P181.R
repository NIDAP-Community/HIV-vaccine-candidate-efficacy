...
Filtered_Study_VB013_P181 <- function(MacacaMetadata) {
    library(tidyverse)
    tab <- MacacaMetadata
    if(TRUE){
        x=paste0("^", c("P181","VB013"), "$", collapse = "|")    
    }
    else{
        x=paste(c("P181","VB013"),collapse="|")
    }
    if(TRUE){
...
print("template_function_Filtered_Study_VB013_P181.R #########################################################################")
...
var_Filtered_Study_VB013_P181<-Filtered_Study_VB013_P181(var_MacacaMetadata)
invisible(graphics.off())
saveRDS(var_Filtered_Study_VB013_P181, paste0(rds_output,"/var_Filtered_Study_VB013_P181.rds"))
