...
P181_LCF <- function(Normalized_Data, Meta_P181) {
    ...
    if (TRUE) {
        ...
        metadata <- dplyr::collect(Meta_P181)
    ...
    Sampinfo <- dplyr::collect(Meta_P181)
...
print("template_function_P181_LCF.R #########################################################################")
...
var_P181_LCF<-P181_LCF(var_Normalized_Data,var_Meta_P181)
invisible(graphics.off())
saveRDS(var_P181_LCF, paste0(rds_output,"/var_P181_LCF.rds"))
