...
DEG_Age_IGF_neg <- function(Filter_low_count_IGFneg, Select_wk13_IGFneg) {
    suppressMessages(library(limma))
    suppressMessages(library(tidyverse))
    suppressMessages(library(edgeR))
    suppressMessages(library(stringr))
    df <- Filter_low_count_IGFneg 
    ...
    samples_for_deg_analysis = c("H02V_wk13","H21Y_wk13","H22D_wk13","H22E_wk13","H22F_wk13","H24D_wk13","H24H_wk13","H28T_wk13","H28V_wk13","H28Y_wk13","H35H_wk13","H35R_wk13","R957_wk13","R958_wk13","R960_wk13","R962_wk13","R963_wk13","R966_wk13","R968_wk13","R976_wk13","T114_wk13","T115_wk13","T121_wk13","T123_wk13","T125_wk13","T126_wk13","T127_wk13","T128_wk13")
    targetfile <- Select_wk13_IGFneg
    contrasts_param<-c("P181-VB013")
    sample_name_column_param<-"sampleID"
    ordered_covariates=c("STUDY")
    return_mean_and_sd_param<-TRUE
    contrast_variable_param<-"STUDY"
...
print("template_function_DEG_Age_IGF_neg.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_Filter_low_count_IGFneg<-readRDS(paste0(rds_output,"/var_Filter_low_count_IGFneg.rds"))
Input_is_Seurat_count <- 0
for(item in var_Filter_low_count_IGFneg){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_Filter_low_count_IGFneg<-as.data.frame(var_Filter_low_count_IGFneg)}else{var_Filter_low_count_IGFneg <- var_Filter_low_count_IGFneg}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
...
var_DEG_Age_IGF_neg<-DEG_Age_IGF_neg(var_Filter_low_count_IGFneg,var_Select_wk13_IGFneg)
invisible(graphics.off())
saveRDS(var_DEG_Age_IGF_neg, paste0(rds_output,"/var_DEG_Age_IGF_neg.rds"))
